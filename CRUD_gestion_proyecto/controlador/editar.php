<?php
    ...
    $sql =  "UPDATE proyecto SET titulo = :titulo, prenda = :prenda, periodo = :periodo, curso = :curso, fecha = :fecha, nota = :nota, foto = :foto WHERE id_proyecto = :id";
    
    try {
        $update = $conexion -> prepare($sql);
        $update ->bindParam(':id', $id, PDO::PARAM_INT);
        $update -> bindParam(':titulo', $titulo, PDO::PARAM_STR);
        $update -> bindParam(':prenda', $descripcion, PDO::PARAM_STR);
